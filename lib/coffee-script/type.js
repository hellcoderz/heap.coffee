...
(function() {
  var ARITH, Access, Arr, ArrowType, Assign, BITWISE, Base, Binding, Block, COMPARE, Call, Cast, Code, Comment, DeclareType, F32, F64, FP, FREE, FreshLiteral, HEAP, I16, I32, I8, If, Index, Literal, MALLOC, MEMCPY, Obj, Op, Param, Parens, PointerType, PrimitiveType, Return, SP, SPREAL, Scope, StructType, Switch, Try, TypeArr, TypeAssign, TypeName, TypeObj, TypeObjField, U16, U32, U8, Value, ViewLiteral, While, alignFrame, alignOffset, alignmentUnits, anyPtrTy, argOffsetExpr, binopType, cast, coerceIntegral, collectReturns, declare, error, extend, f64ty, flatten, frameOffsetExpr, i16ty, i32ty, i8ty, inlineMemcpy, integral, last, lastNonCommentOrDecl, makeDeref, multExpr, newTypedScope, normalizePtr, numeric, offsetExpr, opts, printWarn, putOnStack, requireExpr, stackFence, stackOffsetExpr, typeTransformArray, tystr, u16ty, u32ty, u8ty, undefExpr, unify, unit, _ref, _ref1,
  ...
  ViewLiteral = (function(_super) {
    ...
    ViewLiteral.prototype.compileNode = function(o) {
      var name, scope, _ref2;
      scope = o.scope;
      if (!((_ref2 = scope.freshView) != null ? _ref2[this.value] : void 0)) {
        if (scope.freshView == null) {
          scope.freshView = {};
        }
        name = scope.freshView[this.value] = o.scope.freeVariable(this.value);
        scope.assign(name, "" + (HEAP.compile(o)) + "." + this.value);
      }
      return scope.freshView[this.value];
    };
  ...
  SP = function() {
    ...
    sp.fixup = function(frameSize) {
      var asn;
      asn = new Assign(SPREAL, new Value(new Literal(frameSize)), '-=');
      return this.base.SPREAL = new Value(new Parens(new Block([asn])));
    };
    sp.base.SPREAL = SPREAL;
    sp.base.compileNode = function(o) {
      var scope, spreal;
      scope = o.scope;
      this.value = scope.freeVariable(this.value);
      spreal = normalizePtr(this.SPREAL, U8.BYTES_PER_ELEMENT, I32.BYTES_PER_ELEMENT);
      scope.assign(this.value, spreal.compile(o));
  ...
  FP = function() {
    ...
    fp.fixup = function(o, frameSize) {
      return this.base = new Parens(new Block([new Op('+', o.SP, new Value(new Literal(frameSize >> 2)))]));
    };
    return fp;
  };
  ...
  offsetExpr = function(op, base, baseTy, offset, offsetTy) {
    ...
    if (offset !== 0) {
      return new Op(op, normalized, new Value(new Literal(offset / oalign)));
    } else {
      return normalized;
    }
  };
  ...
  stackOffsetExpr = function(o, offset, offsetTy) {
    return offsetExpr('+', o.SP, i32ty, offset, offsetTy);
  };

  frameOffsetExpr = function(o, offset, offsetTy) {
    return offsetExpr('-', o.FP, i32ty, offset, offsetTy);
  };

  argOffsetExpr = function(o, offset, offsetTy) {
    return offsetExpr('-', o.SP, i32ty, offset, offsetTy);
  };
  ...
  declare = function(scope, v, ty, variables, param, subpattern) {
    ...
    if (v instanceof Arr) {
      ...
      for (i = _i = 0, _len = els.length; _i < _len; i = ++_i) {
        ...
        if (el["this"]) {
          ...
          declare(scope, el.base, ty.elements[i], variables, param, true);
        }
      }
      return;
    }
    if (v instanceof Obj) {
      ...
      for (_j = 0, _len1 = objs.length; _j < _len1; _j++) {
        ...
        if (obj instanceof Assign) {
          ...
          declare(scope, obj.value, (_ref2 = ty.names[name.value]) != null ? _ref2.type : void 0, variables, param, true);
        } else if (obj["this"]) {
          error(v, "cannot type @-names");
        } else {
          name = obj.base;
          declare(scope, name, (_ref3 = ty.names[name.value]) != null ? _ref3.type : void 0, variables, param, true);
        }
      }
      return;
    }
  ...
  putOnStack = function(scope, name) {
    ...
    bind.onStack = true;
    ty = bind.type;
    if (bind.param && ty instanceof StructType) {
  ...
  Param.prototype.declareType = function(scope, o) {
    var _ref2;
    declare(scope, this.name, (_ref2 = this.type) != null ? _ref2.lint(o.types) : void 0, (this.variables = []), true);
    return scope;
  };
  ...
  alignFrame = function(scope) {
    return alignOffset(scope.frameVarsSize, f64ty) + alignOffset(scope.frameArgsSize, f64ty);
  };
  ...
  Literal.prototype.typeTransformNode = function(o) {
    var bind, i, isStruct, offset, ty, val;
    if (o.value) {
      val = this.value;
      if (val === 'null' || (val === '0' && o.wantsToBePointer)) {
        ...
        if (bind.onStack) {
          ...
          offset = bind.param && isStruct ? frameOffsetExpr(o, bind.offset, ty) : stackOffsetExpr(o, bind.offset, ty);
          if (isStruct) {
  ...
  Call.prototype.typeTransformNode = function(o) {
    var arg, args, aty, calleeFrameOffset, dest, fty, i, paramTys, pty, types, _i, _j, _len, _len1, _ref2, _ref3, _ref4;
  ...
  Code.prototype.typeTransformNode = function(o) {
    ...
    _ref2 = lastNonCommentOrDecl(exprs), implicitReturn = _ref2[0], i = _ref2[1];
    if (!implicitReturn.jumps()) {
      exprs[i] = new Return(implicitReturn);
    }
    if ((rty = this.returnType) instanceof ArrowType) {
      _ref3 = collectReturns(this.body);
      for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
        rexpr = _ref3[_i].expression;
    ...
    if (frameSize = alignFrame(o.scope)) {
      ...
      for (_k = 0, _len2 = _ref5.length; _k < _len2; _k++) {
        ...
        for (_l = 0, _len3 = _ref6.length; _l < _len3; _l++) {
          arg = _ref6[_l];
          if (!(arg.binding.onStack && !(arg.binding.type instanceof StructType))) {
            continue;
          }
  ...
  exports.analyzeTypes = function(root, o) {
    ...
    root.traverseChildren(true, function(node) {
      var op;
      if ((node instanceof DeclareType) || (node instanceof TypeAssign) || (node instanceof Op && node.isUnary() && ((op = node.operator) && op === '&' || op === '*' || op === 'sizeof')) || (node instanceof Code && node.paramTypes)) {
        usesTypes = true;
        return false;
      }
    });
    ...
    if (frameSize = alignFrame(o.scope)) {
      stackFence(exprs, frameSize, true);
      o.SP.fixup(frameSize);
    }
    root.typeTransform(o);
    if (Scope.root.needsMalloc) {
      obj = new Value(new Obj([new Assign(new Value(new Literal('malloc')), MALLOC, 'object'), new Assign(new Value(new Literal('free')), FREE, 'object')]));
      exprs.unshift(new Assign(obj, requireExpr('heap/malloc')));
    }
    root.transformRoot = function(o) {
      return o.scope.assign(HEAP.compile(o), "require('heap/heap')");
    };
    return root;
  };
